...
public class EDIFACTLexicalScanner extends EDILexicalScanner {
	 ...
	 *            delimiter
	 */
	public EDIFACTLexicalScanner(final StringBuffer inputDocument, final EDIFACTDelimiters delimiter) {
	...
	protected IToken getNextToken(final int position) {
		...
		if (getEDIFACTDelimiter().containsDelimiter(str)) {
			return new Token(str, position, getEDIFACTDelimiter().getDelimiterIdentifier(str));
		}
		return new Token(str, position);
	}
